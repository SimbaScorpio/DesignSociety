	...
	public class MyRichAI : AIBase
	{
		...
		protected override Vector3 ClampToNavmesh (Vector3 position)
		{
			if (richPath != null) {
				...
				if (funnel != null)
					return funnel.ClampToNavmesh (position);
			}
			return position;
		}
...
//			yield return new WaitForSeconds(Mathf.Abs(anim[link.clip].length/link.animSpeed));
			...
			while (Vector3.Distance (transform.position, targetPos) > 0.1f) {
				if (al.oneWay) {
					while (!RotateTowards2 (direction))
						yield return null;
				} 
				...
				if (Vector3.Dot (currentDirection, direction) <= 0)
					...
					if (al.oneWay)
						...
						transform.position += direction * Time.deltaTime * espeed;
				}
				yield return null;
			}
		...
		bool RotateTowards2 (Vector3 trotdir)
		{
			trotdir.y = 0;
			if (trotdir != Vector3.zero) {
				...
				eul.y = Mathf.MoveTowardsAngle (eul.y, trot.y, rotationSpeed * Time.deltaTime);
