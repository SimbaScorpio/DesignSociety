		...
		public void ClearMessages ()
		{
			...
			for (int i = 0; i < displayList.Count; ++i) {
				if (displayList [i] != null) {
					Remove (displayList [i]);
				}
				displayList.RemoveAt (i--);
			}
		}

		bool IsRepeat (GameObject blob, int type)
		{
			// avatar
			if (type == 1) {
				string name1 = blob.GetComponent<UITempNameSaver> ().text;
				for (int i = 0; i < displayList.Count; ++i) {
					GameObject obj = displayList [i];
					if (objToType [obj] == 1) {
						string name2 = obj.GetComponent<UITempNameSaver> ().text;
						if (name1 == name2)
							return true;
					}
				}
			} else if (type == 2) {
				string keyword1 = blob.GetComponentsInChildren<Text> () [1].text;
				for (int i = 0; i < displayList.Count; ++i) {
					GameObject obj = displayList [i];
					if (objToType [obj] == 2) {
						string keyword2 = obj.GetComponentsInChildren<Text> () [1].text;
