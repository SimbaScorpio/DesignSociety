...
namespace DesignSociety
{
	...
	public class StorylinePart : MonoBehaviour
	{
		...
		public void Restart ()
		{
			if (storyline.characterdata == null || storyline.characterdata.Length == 0)
				return;
			if (spotNames == null) {
				InitSpotNames ();
			}
		...
		void InitSpotNames ()
		{
			...
			if (storyline.scenedata == null) {
				Log.warn ("缺少场景的故事线：" + fileName);
				return;
			}
			foreach (SceneData scene in storyline.scenedata) {
				string spotName = scene.spot_name;
				if (spotName == null || !sm.nameToScene.ContainsKey (spotName))
					continue;	// 该场景不合格
...
//			}
		...
		bool RandomlyArrangeJobCandidates ()
		{
			...
			foreach (InitCharacterData initdata in storyline.initcharacterdata) {
				...
				if (jobName == null || !sm.nameToJob.ContainsKey (jobName))
					continue;	// 该岗位不合格
		...
		bool RecursivelyArrangeJobCandidate (List<string> jobNames, int i)
		{
			...
			foreach (string chaName in job.candidates) {
				if (chaName == null || !sm.nameToCharacter.ContainsKey (chaName)) {
