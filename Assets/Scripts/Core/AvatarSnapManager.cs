...
namespace DesignSociety
{
	public class AvatarSnapManager : MonoBehaviour
	{
		...
		{
			instance = this;
			actions.Add (StandActionName.stand_hello.ToString ());
			actions.Add (StandActionName.stand_wave_hand.ToString ());
			actions.Add (StandActionName.stand_nod.ToString ());
			actions.Add (StandActionName.stand_shake_head.ToString ());
		}
		...
		RenderTexture CreateAvatar (CharacterData cd)
		{
			...
			CharacterGenerator.GetInstance ().Generate (man, cd, false);
			man.GetComponent<Animator> ().Play (actions [Random.Range (0, actions.Count)], 0, 0f);
		...
		{
			// 如果已经在显示，那么只需返回当前投影
			if (cd.name != null && nameToAvatar.ContainsKey (cd.name)) {
		...
		{
			if (name != null && nameToAvatar.ContainsKey (name)) {
