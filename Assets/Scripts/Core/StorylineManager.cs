...
namespace DesignSociety
{
	...
	public class StorylineManager : MonoBehaviour
	{
		...
		{
			if (part != null && !storylineParts.Contains (part)) {
		...
		void InitializeCharacters ()
		{
			foreach (StorylinePart part in storylineParts) {
				...
				foreach (CharacterData cha in part.storyline.characterdata) {
					...
					if (cha.name != null && nameToCharacter.ContainsKey (cha.name)) {
		...
		void InitializeCompositeMovements ()
		{
			foreach (StorylinePart part in storylineParts) {
				...
				foreach (CompositeMovementData com in part.storyline.compositemovementdata) {
					...
					if (com.name != null && nameToCompositeMovement.ContainsKey (com.name)) {
		...
		void InitializeJobs ()
		{
			foreach (StorylinePart part in storylineParts) {
				...
				foreach (InitCharacterData job in part.storyline.initcharacterdata) {
					...
					if (job.name != null && nameToJob.ContainsKey (job.name)) {
		...
		void InitializeScenes ()
		{
			foreach (StorylinePart part in storylineParts) {
				...
				foreach (SceneData spot in part.storyline.scenedata) {
					...
					if (spot.principal == null || !nameToJob.ContainsKey (spot.principal)) {
						Log.error ("初始化场景【" + spot.spot_name + "】失败：未定义的主要岗位【" + spot.principal + "】");
						continue;
					}
					if (spot.spot_name != null && nameToScene.ContainsKey (spot.spot_name)) {
		...
		{
			if (spotName == null || !nameToStorylinePart.ContainsKey (spotName))
				return "";
		...
		void InitRandomActivity ()
		{
			...
			for (int i = 0; i < storylineParts.Count; ++i) {
				...
				for (int j = 0; j < storyline.random.Length; ++j) {
					...
					if (temp.randomscene != null && nameToRandomActivity.ContainsKey (temp.randomscene)) {
		...
		void InitRandomAction (RandomActivity t)
		{
			...
			for (int i = 0; i < t.randomaction.Length; ++i) {
				...
				if (temp.label != null && dict.ContainsKey (temp.label)) {
		...
		void InitRandomLandmark (RandomActivity t)
		{
			...
			for (int i = 0; i < t.randomlocation.Length; ++i) {
				...
				if (landmark != null && dict.ContainsKey (landmark)) {
		...
		void InitRandomPerson (RandomActivity t)
		{
			for (int i = 0; i < t.randomperson.Length; ++i) {
				...
				if (pname == null || !nameToCharacterObj.ContainsKey (pname)) {
		...
		{
			if (randomscene == null || !nameToLandmarkDict.ContainsKey (randomscene)
			...
			if (landmark.m_label != null && nameToActionDict [randomscene].ContainsKey (landmark.m_label))
