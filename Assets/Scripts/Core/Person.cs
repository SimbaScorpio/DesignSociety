...
namespace DesignSociety
{
	public class Person : MonoBehaviour
	{
		...
		void Start ()
		{
			...
			actionDealer.ApplyIdleAction (Random.Range (0, 2f));
		}
		...
		PrincipalActivity tempPActivity;
		SecondPerson tempSOther;
		FollowingActivity tempFActivity;
		ThirdPerson tempTOther;
		int tempIndex;
		Person tempWaitPerson;

		void AssignNewPrincipalActivity ()
		{
			tempPActivity = principalActivities [0];
			if (isValidCompositeAction (tempPActivity.position, tempPActivity.action)) {
				for (int i = 0; i < tempPActivity.other_people.Length; ++i) {
					tempSOther = tempPActivity.other_people [i];
					if (tempSOther.name != null && storylineManager.nameToJob.ContainsKey (tempSOther.name)) {
						tempIndex = 0;
						for (tempIndex = 0; tempIndex < secondChildren.Count; ++tempIndex) {
							if (secondChildren [tempIndex].name == tempSOther.name)
								break;
						}
						if (tempIndex < secondChildren.Count) {
							Log.warn ("Spot [" + spotName + "]'s principal activity [" + tempPActivity.description + "] job [" + tempSOther.name + "] overlapped: skipped");
						} else {
							string realName = storylineManager.nameToJobCandidateName [tempSOther.name];
							GameObject realObj = storylineManager.nameToCharacterObj [realName];
							Person realScript = realObj.GetComponent<Person> ();
							secondChildren.Add (tempSOther);
							waitToBeChildren.Add (realScript);
							children.Add (realScript);
						}
					} else {
						Log.warn ("Spot [" + spotName + "]'s principal activity [" + tempPActivity.description + "] has undefined job [" + tempSOther.name + "]: skipped");
					}
				}
				currentPrincipalActivity = tempPActivity;
				compositeMovement = GetCompositeMovementData (tempPActivity.position, tempPActivity.action);
				state = ComState.ARRIVING;
				StartCoroutine (WaitToBeMySecondChild ());
			} else {
				Log.warn ("Spot [" + spotName + "]'s principal activity [" + tempPActivity.description + "] has undefined composite movement [" + tempPActivity.action + "]: skipped");
			}
		...
		IEnumerator WaitToBeMySecondChild ()
		{
			while (waitToBeChildren.Count > 0) {
				for (int i = 0; i < waitToBeChildren.Count; ++i) {
					tempWaitPerson = waitToBeChildren [i];
					if (!tempWaitPerson.isPrincipal && !tempWaitPerson.isBeingControlled &&
					    (tempWaitPerson.parent == null || tempWaitPerson.parent == this)) {
						tempWaitPerson.Stop ();
						tempWaitPerson.parent = this;
						tempWaitPerson.isBeingControlled = true;
						tempWaitPerson.spotName = spotName;
						storylineManager.JoinStorylineSpot (tempWaitPerson.name, spotName);
		...
		{
			tempFActivity = followingActivities [0];
			if (isValidCompositeAction (tempFActivity.position, tempFActivity.action)) {
				for (int i = 0; i < tempFActivity.other_people.Length; ++i) {
					tempTOther = tempFActivity.other_people [i];
					if (tempTOther.name != null && storylineManager.nameToJob.ContainsKey (tempTOther.name)) {
						int index = 0;
						for (index = 0; index < thirdChildren.Count; ++index) {
							if (thirdChildren [index].name == tempTOther.name)
								break;
						}
						if (index < thirdChildren.Count) {
							Log.warn ("Spot [" + spotName + "]'s following activity [" + tempFActivity.description + "] job [" + tempTOther.name + "] overlapped: skipped");
						} else {
							string realName = storylineManager.nameToJobCandidateName [tempTOther.name];
							GameObject realObj = storylineManager.nameToCharacterObj [realName];
							Person realScript = realObj.GetComponent<Person> ();
							thirdChildren.Add (tempTOther);
							waitToBeChildren.Add (realScript);
							children.Add (realScript);
						}
					} else {
						Log.warn ("Spot [" + spotName + "]'s following activity [" + tempFActivity.description + "] has undefined job [" + tempTOther.name + "]: skipped");
					}
				}
				currentFollowingActivity = followingActivities [0];
				compositeMovement = GetCompositeMovementData (tempFActivity.position, tempFActivity.action);
				state = ComState.ARRIVING;
				StartCoroutine (WaitToBeMyThirdChild ());
			} else {
				Log.warn ("Spot [" + spotName + "]'s following activity [" + tempFActivity.description + "] has undefined composite movement [" + tempFActivity.action + "]: skipped");
			}
		...
		bool IsAtProperLocation (Self self, Person person, bool doAction)
		{
			...
			case 1:	// labeled location
				...
				if (tempDestination == null)
					return true;
				if (Vector3.Distance (person.transform.position, tempDestination.position) <= distanceError && Mathf.Abs (person.transform.rotation.eulerAngles.y - tempDestination.rotation.eulerAngles.y) % 360 < rotationError) {
					if (doAction) {
		...
		void DisplayBubble (Person person, string[] bubble_content, int bubble_direction, ref int index)
		{
			...
			switch (bubble_type) {
			case 1:
				person.bubbleDealer.NetworkChatBubble (content, Global.chatDuration, bubble_direction);
				break;
			case 2:
				person.bubbleDealer.NetworkKeywordBubble (content, Global.keywordDuration);
				break;
			case 3:
				person.bubbleDealer.NetworkIconBubble (content, Global.iconDuration);
				break;
			case 4:
