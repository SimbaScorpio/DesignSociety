...
namespace DesignSociety
{
	public class NetworkPlayerTeleport : NetworkBehaviour
	{
		...
		public GameObject targetMarkPref;
		...
		void Start ()
		{
			if (isLocalPlayer) {
				...
				targetMark = Instantiate (targetMarkPref) as GameObject;
				//targetMarkAnim = ghost.GetComponent<Animator> ();
			}
		}
		...
		void Update ()
		{
			if (!isLocalPlayer)
				return;
			if (targetMark != null) {
				if (Vector3.Distance (transform.position, targetMark.transform.position) < 0.1f) {
					targetMark.transform.position = new Vector3 (1000, 0, 0);
				}
			}
		...
		IEnumerator CountPressTime ()
		{
			...
			while (true) {
				...
				yield return null;
				if (state == 0) {
					if (pressCount > dragPressThreshold) {
						OnTeleportBegin ();
						pressCount = 0;
						state = 4;
...
//					}
		...
		void OnTeleportBegin ()
		{
			...
			UIClientBuildingMask.GetInstance ().Show ();
			UIClientTeachingPanel.GetInstance ().OnTeleporting ();
		...
		{
			UIClientBuildingMask.GetInstance ().Hide ();
			UIClientTeachingPanel.GetInstance ().OnLanding ();
		...
		void OnTeleportEnd ()
		{
			...
			UIClientMessageMenu.GetInstance ().ClearMessages ();
			LocationDisplay ();
		}
		...
		void CheckPressUpSpecial ()
		{
			if ((state == 0 || state == 4 || state == 5) &&
			    pressCount != 0 &&
			    pressCount < clickPressThreshold &&
			    Vector3.Distance (Input.mousePosition, startMousePosition) < 20.0f)
				OnClick ();
			else {
				if (state == 1) {
					state = 4;
				...
				if (ad.IsWalking () && IsPressing (this.gameObject)) {
					...
					if (!RaycastObstacle ()) {
						if (ad.IsWalking () || !ad.IsPlaying ()) {
							...
							//if (targetMark != null) {
							//targetMark.transform.position = AstarPath.active.GetNearest (mouseLandingPos).position;
							//targetMarkAnim.Play ("MoveTargetMark", 0, 0);
							//}
		...
		void SwitchGhostToPlayer ()
		{
			...
			Vector3 landingPos = GetAvailableLandPos ();
		...
		// Method: hit ray from screen step by step along y-axis, until reachable point hit
		Vector3 GetAvailableLandPos ()
		{
			...
			Vector3 tryhitPos = RaycastPointWithMouse (pos, ref layerCheck);
			while (!layerCheck || !IsLandingPosAvailable (tryhitPos, ref target)) {
				pos.y -= deltay;
				if (pos.y <= 0)
					break;
				tryhitPos = RaycastPointWithMouse (pos, ref layerCheck);
			}
			return target;
		}

		bool IsLandingPosAvailable (Vector3 pos, ref Vector3 target)
		{
			AstarPath.active.FloodFill ();
			GraphNode areaNode = AstarPath.active.GetNearest (pos).node;
			Vector3 nearPos = (Vector3)areaNode.position;
			//if (Vector3.Distance (nearPos, pos) > 1f)
			//	return false;
		...
		int[] indexes = new int[10] { 1, 392, 379, 580, 553, 544, 561, 615, 634, 109 };
		...
		Vector3 RaycastPointWithMouse (Vector3 mousePos, ref bool flagCheck)
		{
			Ray ray = cameraEditor.ScreenPointToRay (mousePos);
			RaycastHit hit;
			if (Physics.Raycast (ray, out hit, float.MaxValue)) {
				if (hit.collider.gameObject.layer == LayerMask.NameToLayer ("FloorMask"))	// FloorMask
					flagCheck = true;
				else
					flagCheck = false;
				return hit.point;
		...
		UIClientBuildingIntro uibuilding;
