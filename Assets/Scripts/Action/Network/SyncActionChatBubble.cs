...
namespace DesignSociety
{
	...
	public class SyncActionIconBubble : SyncActionBubble
	{
		protected override void OnSetContent (Transform bubble, string content)
		{
			...
			if (texture != null) {
				...
				SpriteRenderer renderer = bubble.GetComponentInChildren<SpriteRenderer> ();
	...
	public class SyncActionKeywordBubble : SyncActionBubble
	{
		protected override void OnSetContent (Transform bubble, string content)
		{
			...
			int num = content.Length;
			SpriteRenderer[] sr = bubble.GetComponentsInChildren<SpriteRenderer> ();
			sr [0].size = new Vector2 (num * 0.5f + 1f, sr [0].size.y);
			if (sr.Length > 1)
				sr [1].transform.localPosition = new Vector3 ((num * 0.5f + 1f) * 0.8f - 0.5f, 2f, 0);

			if (GetComponent<Person> ().isActiveAndEnabled) {
				Transform tr = bubble.transform.Find ("root/indicator");
				if (tr != null)
