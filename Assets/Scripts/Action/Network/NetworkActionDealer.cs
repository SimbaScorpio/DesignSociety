...
namespace DesignSociety
{
	public class NetworkActionDealer : NetworkBehaviour, IActionCompleted
	{
		...
		private float possibility = 0.02f;
		...
		public void ApplyIdleAction (float duration)
		{
			ActionIdle idle = gameObject.AddComponent<ActionIdle> ();
			idle.Setting (gameObject, "", duration, null);
		}
		...
		public void ApplyWalkAction (Landmark landmark, bool turn, IActionCompleted callback)
		{
			...
			} else if (lastStateName == "walk_middle_blend_tree" || lastStateName == "stand_middle_hold" || lastStateName == "stand_middle_pickup_table" || lastStateName == "stand_middle_pickup_bag" || lastStateName == "stand_middle_pickup_ground") {
		...
		public void RealCallingStop ()
		{
			if (landmark != null) {
				callingStop = true;
			} else if (IsWalking ()) {
				actionWalker.RealStop ();
			}
		}
